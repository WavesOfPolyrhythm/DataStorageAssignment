...
public class EmployeeService(IEmployeeRepository employeeRepository, IRolesRepository rolesRepository) : IEmployeeService
{
    ...
    public async Task<EmployeeModel?> UpdateEmployeeAsync(EmployeeUpdateForm form)
    {
        try
        {
            ...
            /// 
            /// The code fetches the role from the repository using the provided RoleId. 
            ...
            var role = await _rolesRepository.GetAsync(x => x.Id == form.RoleId);
            if (role == null)
            {
